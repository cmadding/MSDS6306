...
#Load teh update data

ausair2 = read.csv("ausair.csv",header = TRUE)

#Holt's Linear Trend Model
fit1h = holt(air, alpha = .8, beta = .2, initial = "optimal", h = 13)
...
fit3h = holt(air, alpha = .8, beta = .2, damped = TRUE, initial = "optimal", h = 13)
lines(fitted(fit3h), col = "darkgreen", type= "o")
lines(fit3h$mean,col = "darkgreen", type= "o")

fit4h = holt(air, alpha = .8, beta = .2, damped = TRUE, initial = "optimal", exponential = TRUE, h = 13)
lines(fitted(fit4h), col = "cyan", type= "o")
lines(fit4h$mean,col = "cyan", type= "o")

ausair2ts <- ts(ausair2$Passengers, start = c(1970,1), frequency = 1)
accuracy(fit1h, ausair2ts)
accuracy(fit2h, ausair2ts)
accuracy(fit3h, ausair2ts)
accuracy(fit4h, ausair2ts)
...
#Breakout notes
library(readr)
AirlinePasngr <- read_csv("AirlinePasngr.csv")
#convert to a Time Series
AirlinePasngrTS <- ts(AirlinePasngr$Passengers, start = c(1949,1), frequency = 12)

AirlinePasngrTS1 = window(AirlinePasngrts, end = 1958)
AirlinePasngrTS
AirlinePasngrTS1

#Holt's Linear Trend Model
ALfit1h = hw(AirlinePasngrTS1,seasonal = "additive", h = 35)
ALfit2h = hw(AirlinePasngrTS1,seasonal = "multiplicative", h = 35)

accuracy(ALfit1h, AirlinePasngrTS)

AirlinePasngrts1957.fit3 = ses(AirlinePasngrts1957, h = 3) #defaults

accuracy(AirlinePasngrts1957.fit3, AirlinePasngrts)
